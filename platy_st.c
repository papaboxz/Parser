...
/* __DATE__, __TIME__, __LINE__, __FILE__, __STDC__ are predefined preprocessor macros*/
  if(ansi_c){
    ...
    printf("ERROR: Compiler is not ANSI C compliant!\n");
    exit(1);
  }
...
/* __DATE__, __TIME__, __LINE__, __FILE__ are predefined preprocessor macros*/
       printf("Date: %s  Time: %s",__DATE__, __TIME__);
       printf("Runtime error at line %d in file %s", __LINE__, __FILE__);
       printf("%s%s%s",argv[0],": ","Missing source file name.");
       printf("%s%s%s","Usage: ", "scanner", "  source_file_name");
       exit(1);
	}	
 ...
 /* create a source code input buffer - multiplicative mode */	
	...
	if (sc_buf == NULL){
	  printf("%s%s%s",argv[0],": ","Could not create source buffer");
	  exit(1);
	}

/*open source file */
	if ((fi = fopen(argv[1],"r")) == NULL){
		printf("%s%s%s%s",argv[0],": ", "Cannot open file: ",argv[1]);
		exit (1);
	}
/* load source file into input buffer  */
	 ...
	 if (loadsize == RT_FAIL_1)
       printf("%s%s%s",argv[0],": ","Error in loading buffer.");
